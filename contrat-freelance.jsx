...
/* ── BASE DE DONNÉES (mock local — remplacer par Supabase en prod) ── */
let _mockUser = null;
let _mockContracts = [];
let _mockPlan = { plan: "free", contracts_used: 0 };
...
const supabase = {
  auth: {
    getSession: async () => ({ data: { session: _mockUser ? { user: _mockUser } : null } }),
    onAuthStateChange: (cb) => {
      // pas de vrai listener en mode mock
      return { data: { subscription: { unsubscribe: () => {} } } };
    },
    signUp: async ({ email }) => {
      _mockUser = { id: "mock-uid", email };
      return { error: null };
    },
    signInWithPassword: async ({ email, password }) => {
      if (password.length < 3) return { error: { message: "Invalid login credentials" } };
      _mockUser = { id: "mock-uid", email };
      return { error: null };
    },
    signOut: async () => { _mockUser = null; _mockPlan = { plan: "free", contracts_used: 0 }; _mockContracts = []; },
    getUser: async () => ({ data: { user: _mockUser } }),
  ...
  from: (table) => ({
    select: (cols) => ({
      order: (col, opts) => ({
        then: (resolve) => {
          // Filtre strict par user_id — retourne uniquement les données de l'utilisateur connecté
          const userId = _mockUser?.id;
          const filtered = table === "contracts"
            ? _mockContracts.filter(r => r.user_id === userId)
            : [];
      ...
      eq: (col, val) => ({
        single: async () => {
          if (table === "users") return { data: _mockPlan, error: null };
    ...
    insert: (row) => ({
      select: () => ({
        single: async () => {
          ...
          if (table === "contracts") _mockContracts.unshift(newRow);
          return { data: newRow, error: null };
        },
      }),
    }),
    update: (vals) => {
      // Stocke les filtres pour appliquer la double condition (id + user_id)
      const filters = {};
      const chain = {
        eq: (col, val) => {
          filters[col] = val;
          return {
            eq: (col2, val2) => {
              ...
              return {
                then: (resolve) => {
                  if (table === "contracts") {
                    // Applique la mise à jour uniquement si tous les filtres correspondent
                    _mockContracts = _mockContracts.map(r => {
                      const match = Object.entries(filters).every(([k, v]) => r[k] === v);
                      return match ? { ...r, ...vals } : r;
                    });
                  } else if (table === "users") {
                    Object.assign(_mockPlan, vals);
                  }
                  resolve({ error: null });
                },
                [Symbol.toStringTag]: "Promise",
    ...
    delete: () => ({
      eq: async (col, val) => {
        if (table === "contracts") {
          const userId = _mockUser?.id;
          // Filtre double : id du contrat ET user_id pour éviter toute suppression cross-user
          _mockContracts = _mockContracts.filter(r => !(r[col] === val && r.user_id === userId));
        }
        return { error: null };
      },
    }),
  ...
  rpc: async (fn, args) => {
    if (fn === "increment_contracts_used") _mockPlan.contracts_used += 1;
    return { error: null };
  },
};
...
const getHistory = async () => {
  if (!_mockUser) return [];
  // Filtre strict par user_id — un utilisateur ne voit que ses propres contrats
  const data = _mockContracts.filter(r => r.user_id === _mockUser.id);
  return data.map(rowToEntry);
};

const saveToHistory = async (entry, form) => {
  if (!_mockUser) return null;
  const newRow = {
    id: Date.now().toString(),
    created_at: new Date().toISOString(),
    user_id: _mockUser.id,
    freelance_name: form.freelanceName,
    freelance_activity: form.freelanceActivity,
    freelance_siret: form.freelanceSiret,
    freelance_address: form.freelanceAddress,
    freelance_email: form.freelanceEmail,
    client_name: form.clientName,
    client_company: form.clientCompany,
    client_address: form.clientAddress,
    client_email: form.clientEmail,
    mission_title: form.missionTitle,
    mission_description: form.missionDescription,
    ...
    contract_text: entry.contract,
    signature_status: "none",
    signature_request_id: null,
  };
  _mockContracts.unshift(newRow);
  _mockPlan.contracts_used += 1;
  return newRow;
};

const deleteFromHistory = async (id) => {
  if (!_mockUser) return;
  // Double condition : id du contrat ET user_id — empêche la suppression cross-user
  _mockContracts = _mockContracts.filter(r => !(r.id === id && r.user_id === _mockUser.id));
};

const getUserPlan = async () => {
  if (!_mockUser) return { plan: "free", contractsUsed: 0 };
  return { plan: _mockPlan.plan ?? "free", contractsUsed: _mockPlan.contracts_used ?? 0 };
};
      ...
      <HistoryPage
        ...
        onDuplicate={async (entry) => {
          const dupRow = {
            id: Date.now().toString(),
            created_at: new Date().toISOString(),
            user_id: _mockUser?.id || "mock-uid",
            freelance_name: entry.form?.freelanceName || "",
            freelance_activity: entry.form?.freelanceActivity || "",
            freelance_siret: entry.form?.freelanceSiret || "",
            freelance_address: entry.form?.freelanceAddress || "",
            freelance_email: entry.form?.freelanceEmail || "",
            client_name: entry.form?.clientName || entry.clientName || "",
            client_company: entry.form?.clientCompany || entry.clientCompany || "",
            client_address: entry.form?.clientAddress || "",
            client_email: entry.form?.clientEmail || "",
            mission_title: (entry.missionTitle || entry.form?.missionTitle || "") + " (copie)",
            mission_description: entry.form?.missionDescription || "",
            start_date: entry.form?.startDate || entry.startDate || null,
            end_date: entry.form?.endDate || entry.endDate || null,
            price: entry.price || entry.form?.price || 0,
            payment_terms: entry.form?.paymentTerms || "Comptant",
            revisions: entry.form?.revisions || "2",
            late_payment_penalty: entry.form?.latePaymentPenalty ?? true,
            contract_text: entry.contract || "",
            signature_status: "none",
            signature_request_id: null,
          };
          _mockContracts.unshift(dupRow);
          const hist = await getHistory();
          setHistory(hist);
        }}
          ...
          {magicSent ? (
            ...
            <>
              ...
              <button
                ...
                style={{ ...oauthBtn, marginBottom:22, background:"#25D366", border:"1.5px solid #25D366", color:"#fff", boxShadow:"0 4px 16px rgba(37,211,102,0.28)" }}
                onMouseOver={e => { if (!oauthLoading) { e.currentTarget.style.background="#1ea34b"; e.currentTarget.style.boxShadow="0 8px 28px rgba(37,211,102,0.45)"; }}}
                onMouseOut={e => { e.currentTarget.style.background="#25D366"; e.currentTarget.style.boxShadow="0 4px 16px rgba(37,211,102,0.28)"; }}
              >
                {oauthLoading === "whatsapp" ? (
                ...
                <span style={{ fontWeight:700 }}>💬 Connexion flash par WhatsApp</span>
              </button>
...
function ContractTimeline({ entry }) {
  ...
  const systemEvents = [
    ...
    },
    {
      ...
      iconBg: "#DBEAFE",
      iconColor: "#1D4ED8",
      label: "Contrat consulté via lien magique",
      detail: `Par ${clientName}`,
      status: "done",
      type: "system",
    },
  ];
