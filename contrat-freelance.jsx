...
const supabase = {
  auth: {
    ...
    signUp: async ({ email, password }) => {
      ...
      if (data.error || data.msg) return { error: { message: data.error_description || data.msg || "Erreur inscription" } };
      if (data.session) { _currentSession = data.session; localStorage.setItem("sb_session", JSON.stringify(data.session)); }
      return { error: null };
    },
    signInWithPassword: async ({ email, password }) => {
      ...
      const data = await res.json();
      if (data.error || data.error_description) return { error: { message: data.error_description || "Identifiants invalides" } };
    ...
    getUser: async () => {
      if (!_currentSession?.access_token) return { data: { user: null } };
      const res = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
        headers: supabaseAuthHeaders(_currentSession.access_token),
      });
      const data = await res.json();
      ...
      select: (cols = "*") => ({
        order: (col, opts = {}) => ({
          then: async (resolve) => {
            ...
            const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?select=${cols}&order=${col}.${dir}`, { headers });
            const data = await res.json();
      ...
      insert: (row) => ({
        select: () => ({
          single: async () => {
            ...
            return { data: record, error: record?.code ? record : null };
          },
        }),
      }),
      update: (vals) => {
